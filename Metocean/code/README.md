# Metocean

### List of code samples 

<!-- 50-List of code -->

<!-- [code entry](link) -->
[code_for_using_dataModel.MarineTransport_Metocean.py](https://github.com/smart-data-models/dataModel.MarineTransport/blob/master/Metocean/code/code_for_using_dataModel.MarineTransport_Metocean.py)


<!-- /50-List of code -->

### Contribution
You can raise an [issue](https://github.com/smart-data-models/dataModel.MarineTransport/issues) or submit your [PR](https://github.com/smart-data-models/dataModel.MarineTransport/pulls) on existing data models
