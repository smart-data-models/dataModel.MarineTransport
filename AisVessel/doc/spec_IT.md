...
Descrizione globale: **Schema NGSI-LD per l'entità AisVessel, che rappresenta le informazioni AIS della nave da diverse fonti di dati AIS**  
...
- `aisTypeSummary[string]`: Riepilogo del tipo di nave AIS  
- `alternateName[string]`: Un nome alternativo per questo elemento  
- `averageSpeed[number]`: Velocità media nell'ultimo viaggio  
- `beam[number]`: Larghezza o beam  
- `callSign[string]`: Segnale di chiamata della nave  
- `courseOverGround[number]`: Rotta sul terreno (gradi)  
- `createdAt[date-time]`: Ora di creazione del record (ISO 8601)  
- `currentPortCountry[string]`: Paese del porto attuale  
- `currentPortId[number]`: ID del porto attuale  
- `currentPortName[string]`: Nome del porto attuale  
- `currentPortUnlocode[string]`: UN/LOCODE del porto attuale  
- `dataProvider[string]`: Fornitore di dati AIS  
...
- `deletedAt[date-time]`: Ora di cancellazione logica (ISO 8601)  
- `description[string]`: Una descrizione di questo elemento  
- `destination[string]`: Destinazione della nave  
- `distanceToGo[number]`: Distanza rimanente per la destinazione  
- `distanceToPort[number]`: Distanza dal porto  
- `distanceTravelled[number]`: Distanza percorsa dall'ultimo porto  
- `draught[number]`: Stazza della nave  
- `dsrc[string]`: Fonte AIS (TER, SAT, ROAM)  
- `dwt[number]`: Portata lorda  
- `eta[date-time]`: Ora stimata di arrivo (ISO 8601)  
- `etaCalculated[date-time]`: ETA calcolata (ISO 8601)  
- `etaUpdatedAt[date-time]`: Ora dell'ultimo aggiornamento dell'ETA (ISO 8601)  
- `expectedArrival[boolean]`: La nave è attesa in arrivo  
- `flagCode[string]`: Codice del paese della bandiera  
- `gt[number]`: Stazza lorda  
- `heading[number]`: Direzione in gradi  
- `id[*]`: Identificatore univoco dell'entità  
- `imo[number]`: Numero IMO della nave  
- `lastPortCode[string]`: Codice del porto precedente  
- `lastPortCountry[string]`: Paese dell'ultimo porto  
- `lastPortId[number]`: ID dell'ultimo porto  
- `lastPortName[string]`: Nome dell'ultimo porto  
- `lastPortTime[date-time]`: Ora di partenza dall'ultimo porto (ISO 8601)  
- `lastPortUnlocode[string]`: UN/LOCODE dell'ultimo porto  
- `latitude[number]`: Latitudine in gradi decimali  
- `lfore[number]`: Distanza dalla stazione AIS alla prua  
- `loa[number]`: Lunghezza totale  
- `longitude[number]`: Longitudine in gradi decimali  
- `market[string]`: Mercato commerciale della nave  
- `maxSpeed[number]`: Velocità massima registrata  
- `mmsi[number]`: Numero MMSI della nave  
- `modifiedAt[date-time]`: Ora dell'ultima modifica (ISO 8601)  
- `name[string]`: Nome di questo elemento  
- `navigationStatus[number]`: ID dello stato di navigazione  
- `nextPortCode[string]`: Codice del prossimo porto  
- `nextPortCountry[string]`: Paese del prossimo porto  
- `nextPortId[number]`: ID del prossimo porto  
- `nextPortName[string]`: Nome del prossimo porto  
- `nextPortUnlocode[string]`: UN/LOCODE del prossimo porto  
- `observedAt[date-time]`: Timestamp dell'ultima osservazione (ISO 8601)  
- `owner[array]`: Una lista che contiene una sequenza di caratteri JSON che si riferisce agli ID univoci dei proprietari  
- `portCallNumber[string]`: Numero della chiamata al porto  
- `portCode[string]`: Codice del porto attuale  
- `publishedAt[date-time]`: Ora di pubblicazione del record (ISO 8601)  
- `rot[number]`: Velocità di rotazione  
- `seeAlso[*]`: elenco di URI che puntano a risorse aggiuntive sull'elemento  
- `shipClass[string]`: Classe della nave  
- `shipCountry[string]`: Paese della nave  
- `shipType[string]`: Tipo di nave da AIS  
...
- `speedOverGround[number]`: Velocità sul terreno (nodi)  
- `type[string]`: Tipo di entità NGSI. Deve essere 'AisVessel'  
- `typeName[string]`: Nome del tipo di nave MarineTraffic  
- `utcSeconds[number]`: Secondi UTC del timeslot AIS  
- `vesselInPort[boolean]`: La nave è attualmente in porto  
- `vesselName[string]`: Nome della nave  
- `vesselRef[string]`: Riferimento univoco della nave  
- `wleft[number]`: Distanza dalla stazione AIS al lato di babordo  
- `yearBuilt[number]`: Anno in cui la nave è stata costruita  
<!-- /30-PropertiesList -->  
...
#### Esempio di AisVessel NGSI-v2 chiave-valore    
 
Ecco un esempio di AisVessel in formato JSON come chiave-valore. Questo è compatibile con NGSI-v2 quando si utilizza `options=keyValues` e restituisce i dati di contesto di un'entità individuale.  
...
#### Esempio di AisVessel NGSI-v2 normalizzato    
 
Ecco un esempio di AisVessel in formato JSON come normalizzato. Questo è compatibile con NGSI-v2 quando non si utilizzano opzioni e restituisce i dati di contesto di un'entità individuale.  
<details><summary><strong>mostra/nascondi esempio</strong></summary>    
...
#### Esempio di AisVessel NGSI-LD chiave-valore    
 
Ecco un esempio di AisVessel in formato JSON-LD come chiave-valore. Questo è compatibile con NGSI-LD quando si utilizza `options=keyValues` e restituisce i dati di contesto di un'entità individuale.  
<details><summary><strong>mostra/nascondi esempio</strong></summary>    
...
#### Esempio di AisVessel NGSI-LD normalizzato    
 
Ecco un esempio di AisVessel in formato JSON-LD come normalizzato. Questo è compatibile con NGSI-LD quando non si utilizzano opzioni e restituisce i dati di contesto di un'entità individuale.  
<details><summary><strong>mostra/nascondi esempio</strong></summary>    
