...
<!-- /15-License -->  
<!-- 20-Description -->  
グローバルな説明: **データモデルは、港湾コミュニティが次の日々に焦点を当てるべき重要な船舶についての情報を提供することを目的としています。各船舶のプロパティを表現することができます: 静的および動的情報**  
...
- `address[object]`: 郵便アドレス。モデル: [https://schema.org/address](https://schema.org/address)	- `addressCountry[string]`: 国。例: スペイン。モデル: [https://schema.org/addressCountry](https://schema.org/addressCountry)  
	- `addressLocality[string]`: 街道アドレスが存在する地域および地域。モデル: [https://schema.org/addressLocality](https://schema.org/addressLocality)  
	...
	- `streetAddress[string]`: 街道アドレス。モデル: [https://schema.org/streetAddress](https://schema.org/streetAddress)  
	- `streetNr[string]`: 公共の通りで特定のプロパティを識別する番号    
- `alternateName[string]`: このアイテムの別名  - `areaServed[string]`: サービスまたは提供されたアイテムが提供される地理的なエリア。モデル: [https://schema.org/Text](https://schema.org/Text)- `callSign[string]`: 海事呼出符号は、船舶に割り当てられた一意の識別子です。モデル: [https://schema.org/Text](https://schema.org/Text)- `courseOverGround[number]`: 地上を越えたコース (COG)。モデル: [https://schema.org/Number](https://schema.org/Number)- `createdDate[date-time]`: エンティティの作成日時は、ISO 8601 UTC形式で表されます。モデル: [https://schema.org/Text](https://schema.org/Text)- `dataProvider[string]`: 調和されたデータエンティティの提供者を識別する文字シーケンス。モデル: [https://schema.org/Text](https://schema.org/Text)- `dateCreated[date-time]`: エンティティの作成タイムスタンプ。このタイムスタンプは通常、ストレージプラットフォームによって割り当てられます  - `dateModified[date-time]`: エンティティの最後の変更タイムスタンプ。このタイムスタンプは通常、ストレージプラットフォームによって割り当てられます  - `deletedDate[date-time]`: エンティティの削除日時は、ISO 8601 UTC形式で表されます。モデル: [https://schema.org/Text](https://schema.org/Text)- `description[string]`: このアイテムの説明  - `etaAis[date-time]`: AISシステムによって報告される予想到着時間は、ISO 8601 UTC形式で表されます。モデル: [https://schema.org/Text](https://schema.org/Text)- `etaAlgorithm[date-time]`: 艦船の位置に基づいてアルゴリズムによって計算された予想到着時間は、ISO 8601 UTC形式で表されます。モデル: [https://schema.org/Text](https://schema.org/Text)- `flagCode[string]`: 海事呼出符号は、船舶に割り当てられた一意の識別子です。モデル: [https://schema.org/Text](https://schema.org/Text)- `id[*]`: エンティティの一意の識別子  - `imo[number]`: 国際海事機関番号 (世界中で永続的に使用される一意の識別子)。モデル: [https://schema.org/Number](https://schema.org/Number)- `lastPortCode[string]`: 直前の港。コードは、到着する港の直前の港を表します (使用可能な場合)。[EMSWe: DE-005-05] [EDIFACT:LOC-3227-92] [IMO:IMO0076]。モデル: [https://schema.org/Text](https://schema.org/Text)- `latitude[number]`: 艦船の緯度座標  - `location[*]`: 艦船の位置を参照するGeojson。観測日付の日に艦船が配置されたポイントでなければなりません  - `longitude[number]`: 艦船の経度座標  - `mmsi[number]`: 海上移動サービス識別番号 (一時的に割り当てられた一意の識別子、現在の旗国の発行)。モデル: [https://schema.org/Number](https://schema.org/Number)- `modifiedDate[date-time]`: エンティティの最後の変更日時は、ISO 8601 UTC形式で表されます。モデル: [https://schema.org/Text](https://schema.org/Text)- `name[string]`: このアイテムの名前  - `navigationStatus[number]`: 列挙型:'0=エンジンを使用して進行中、1=錨を下ろした、2=指令を受けられない、3=運動能力が制限されている、4=吃水によって制限されている、5=係留されている、6=座礁している、7=漁業に従事している、8=帆走中、9=将来のHSCのためのナビゲーション状態の予約、10=将来のWIGのためのナビゲーション状態の予約、11=将来の使用のために予約、12=将来の使用のために予約、13=将来の使用のために予約、14=AIS-SARTがアクティブ、15=未定義 (デフォルト)。ナビゲーション状態。AIVDM/AIVDOデータ形式'。モデル: [https://schema.org/Number](https://schema.org/Number)- `nextPortCode[string]`: 次の港。コードは、到着する港の直前の港を表します (使用可能な場合)。関連するIALA_S211:nestPortCallCod / IMO。[EMSWe: DE-005-07] [EDIFACT:LOC-3227-61] [IMO:IMO0120]。モデル: [https://schema.org/Text](https://schema.org/Text)- `observedDate[date-time]`: この観測の日時は、ISO 8601 UTC形式で表されます。モデル: [https://schema.org/Text](https://schema.org/Text)- `owner[array]`: 所有者の一意のIDを参照するJSONエンコード文字シーケンスを含むリスト  - `portCallNumber[string]`: MRN形式の港湾コール識別子。最初の要素は港の5文字のUN/LOCODEでなければなりません。次に、年と、港によって割り当てられた年ごとの一意のシーケンス番号 (例: ESHUV202310323)。UN/LOCODEの省略形を使用できます (例: H202310323)。portCallNumberは、訪問の初期段階で割り当てられますが、最初はnullになる可能性があります。国際標準では、[Port Call ID]、[Visit ID]、または[Port Call Coded]とも呼ばれます。詳しくは、[Unlocode](https://unece.org/trade/cefact/unlocode-code-list-country-and-territory) [EMSWe: DG-004/DG-004-01] [EDIFACT:BGM-1004] [IALA_S211:portCallId] [IMO:IMO108+IMO0153]を参照してください。モデル: [https://schema.org/Text](https://schema.org/Text)- `portCallRef[uri]`: 親PortCallエンティティを参照します。[NGSI-MarineTransport.PortCall.id]  - `portCode[string]`: 国際海事機関の貿易および輸送のための場所コード。詳しくは、[Unlocode](https://unece.org/trade/cefact/unlocode-code-list-country-and-territory) [EMSWe: DE-004-04] [EDIFACT:LOC-3227-153] [IALA_S211:portCode] [IMO:IMO0108]を参照してください。モデル: [https://schema.org/Text](https://schema.org/Text)- `positionAccuracy[number]`: 列挙型:'0=低い (> 10 m; 自動モード; 例: GNSS受信機またはその他の電子位置決定装置のデフォルト)、1=高い (< 10 m; 例: DGNSS受信機の差分モード)。艦船位置精度フラグのコード。モデル: [https://schema.org/Number](https://schema.org/Number)- `seeAlso[*]`: アイテムに関する追加のリソースを参照するURIのリスト  - `source[string]`: エンティティデータの元のソースをURLとして提供する文字シーケンス。ソースプロバイダーの完全なドメイン名またはソースオブジェクトのURLを推奨します  - `speedOverGround[number]`: 地上を越えた速度 (SOG)。モデル: [https://schema.org/Number](https://schema.org/Number)- `type[string]`: NGSIエンティティタイプ。KeyVesselでなければなりません  - `vesselAtPort[boolean]`: 艦船は港内にあり、待機中、港の入り口で指示を待っているなど  。モデル: [https://schema.org/Boolean](https://schema.org/Boolean)- `vesselName[string]`: 艦船名  。モデル: [https://schema.org/Text](https://schema.org/Text)- `vesselRef[uri]`: 親MasterVesselエンティティを参照します。[NGSI-MarineTransport.MasterVessel.id]- urn:mrn:<oid>:portcalls:mastervessel:id:9999  <!-- /30-PropertiesList -->  
...
#### KeyVessel NGSI-v2 キー値の例    
ここでは、キー値としてJSON形式のKeyVesselの例を示します。これは、`options=keyValues`を使用してコンテキストデータを個々のエンティティとして返すNGSI-v2と互換性があります。  
...
#### KeyVessel NGSI-v2 正規化された例    
ここでは、正規化されたJSON形式のKeyVesselの例を示します。これは、オプションを使用しない場合にコンテキストデータを個々のエンティティとして返すNGSI-v2と互換性があります。  
<details><summary><strong>例の表示/非表示</strong></summary>    
...
#### KeyVessel NGSI-LD キー値の例    
ここでは、キー値としてJSON-LD形式のKeyVesselの例を示します。これは、`options=keyValues`を使用してコンテキストデータを個々のエンティティとして返すNGSI-LDと互換性があります。  
<details><summary><strong>例の表示/非表示</strong></summary>    
...
#### KeyVessel NGSI-LD 正規化された例    
ここでは、正規化されたJSON-LD形式のKeyVesselの例を示します。これは、オプションを使用しない場合にコンテキストデータを個々のエンティティとして返すNGSI-LDと互換性があります。  
<details><summary><strong>例の表示/非表示</strong></summary>    
